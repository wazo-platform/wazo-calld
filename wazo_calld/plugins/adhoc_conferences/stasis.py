...
class AdhocConferencesStasis:
    ...
    def on_stasis_start(self, event_objects, event):
        ...
        with self._adhoc_conference_creation_lock:
            try:
                ...
                logger.debug(
                    'adhoc conference %s: creating bridge', adhoc_conference_id
                )
        ...
        logger.debug(
            'adhoc conference %s: bridging participant %s',
            adhoc_conference_id,
            channel_id,
        )
    ...
    def on_channel_entered_bridge(self, channel, event):
        ...
        logger.debug(
            'adhoc conference %s: channel %s entered', adhoc_conference_id, channel_id
        )

        try:
            is_adhoc_conference_host = (
                channel.getChannelVar(variable='WAZO_IS_ADHOC_CONFERENCE_HOST')['value']
                == 'true'
            )
        except ARINotFound:
            logger.error(
                'adhoc conference %s: channel %s hungup too early or variable not found',
                adhoc_conference_id,
                channel_id,
            )
            return
        ...
        if is_adhoc_conference_host:
            ...
            bridge_helper.global_variables.set(
                'WAZO_HOST_TENANT_UUID', host_tenant_uuid
            )

            self._notify_host_of_channels_already_present(
                adhoc_conference_id, event['bridge']['channels'], host_user_uuid
            )

            logger.debug(
                'adhoc conference %s: setting host connectedline', adhoc_conference_id
            )
    ...
    def _notify_host_of_channels_already_present(
        self, adhoc_conference_id, channel_ids, host_user_uuid
    ):
        for channel_id in channel_ids:
            try:
                ...
                logger.error(
                    'adhoc conference %s: participant %s hanged up before host arrived',
                    adhoc_conference_id,
                    channel_id,
                )
                continue
    ...
    def _set_host_connectedline(self, channel_id, adhoc_conference_id):
        try:
            host_caller_id_number = self._ari.channels.getChannelVar(
                channelId=channel_id, variable='CALLERID(number)'
            )['value']
            self._ari.channels.setChannelVar(
                channelId=channel_id,
                variable='CONNECTEDLINE(all)',
                value=f'"Conference" <{host_caller_id_number}>',
            )
        except ARINotFound:
            logger.error(
                'adhoc conference %s: channel %s hungup too early or variable not found when setting connected line',
                adhoc_conference_id,
                channel_id,
            )
            return
    ...
    def on_channel_left_bridge(self, channel, event):
        ...
        try:
            ...
            logger.error(
                'adhoc conference %s: could not retrieve host user uuid when leaving',
                adhoc_conference_id,
            )
            return
        ...
        if bridge_helper.has_lone_channel():
            logger.debug(
                'adhoc conference %s: only one participant %s left, hanging up',
                adhoc_conference_id,
                channel_id,
            )
            ...
            logger.debug(
                'adhoc conference %s: bridge is empty, destroying', adhoc_conference_id
            )
            try:
            ...
            try:
                adhoc_conference_host_channel_id = bridge_helper.global_variables.get(
                    variable='WAZO_HOST_CHANNEL_ID'
                )
            except KeyError:
                logger.error(
                    'adhoc conference %s: could not retrieve host channel id',
                    adhoc_conference_id,
                )
                return
            if adhoc_conference_host_channel_id == channel_id:
                logger.debug(
                    'adhoc conference %s: host %s left, hanging up all participants',
                    adhoc_conference_id,
                    channel_id,
                )
    ...
    def _hangup_all_participants(self, adhoc_conference_id):
        try:
            channel_ids = self._ari.bridges.get(bridgeId=adhoc_conference_id).json[
                'channels'
            ]
        except ARINotFound:
            logger.error(
                'adhoc conference %s: bridge was destroyed too early when destroying',
                adhoc_conference_id,
            )
            return

        logger.debug(
            'adhoc conference %s: found %s participants',
            adhoc_conference_id,
            len(channel_ids),
        )
        for channel_id in channel_ids:
            logger.debug(
                'adhoc conference %s: hanging up participant %s',
                adhoc_conference_id,
                channel_id,
            )
            try:
    ...
    def on_bridge_destroyed(self, bridge, event):
        ...
        try:
            ...
            logger.error(
                'adhoc conference %s: could not retrieve host user uuid when destroying',
                bridge.id,
            )
            return

        try:
            host_tenant_uuid = bridge_helper.global_variables.get(
                'WAZO_HOST_TENANT_UUID'
            )
        except KeyError:
            logger.error(
                'adhoc conference %s: could not retrieve host tenant uuid when destroying',
                bridge.id,
            )
            return
