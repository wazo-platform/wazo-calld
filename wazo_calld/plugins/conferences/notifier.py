...
class ConferencesNotifier:
    ...
    def participant_joined(
        self, tenant_uuid, conference_id, participant, participants_already_present
    ):
        participants_uuids = list(
            Participants(participant, *participants_already_present).valid_user_uuids()
        )
    ...
    def participant_left(
        ...
        for user_uuid_concerned in participants_uuids:
            event = ConferenceUserParticipantLeftEvent(
                conference_id, participant, tenant_uuid, user_uuid_concerned
            )
            self._bus_producer.publish(event)
    ...
    def participant_unmuted(self, conference_id, tenant_uuid, participant):
        event = ConferenceParticipantUnmutedEvent(
            conference_id, participant, tenant_uuid
        )
        self._bus_producer.publish(event)
    ...
    def participant_talk_started(
        self, conference_id, tenant_uuid, participant, participants
    ):
