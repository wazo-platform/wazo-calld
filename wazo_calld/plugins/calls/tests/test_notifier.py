...
class TestCallNotifier(TestCase):
    ...
    def test_user_missed_call_does_not_leak_tenant_uuid_in_content(self):
        payload = {
            'user_uuid': 'user-uuid',
        ...
        assert_that(self._published_event().content, is_not(has_key('tenant_uuid')))

    def test_user_missed_call_without_tenant_uuid(self):
        payload = {'user_uuid': 'user-uuid', 'tenant_uuid': None}
