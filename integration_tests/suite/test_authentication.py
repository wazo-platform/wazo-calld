...
class TestAuthenticationCoverage(IntegrationTest):
    ...
    def test_auth_on_new_user_call(self):
        assert_that(
            calling(self.calld_client.calls.make_call_from_user).with_args(
                'extension'
            ),
            raises(CalldError).matching(has_properties(status_code=401))
        )
    ...
    def test_auth_on_cancel_user_transfer(self):
        assert_that(
            calling(self.calld_client.transfers.cancel_transfer_from_user).with_args('id'),
            raises(CalldError).matching(has_properties(status_code=401))
        )
    ...
    def test_auth_on_complete_user_transfer(self):
        assert_that(
            calling(self.calld_client.transfers.complete_transfer_from_user).with_args('id'),
            raises(CalldError).matching(has_properties(status_code=401))
        )
    ...
    def test_auth_on_get_user_voicemail_folder(self):
        assert_that(
            calling(self.calld_client.voicemails.get_voicemail_folder_from_user).with_args(
                'folder_id'
            ),
            raises(CalldError).matching(has_properties(status_code=401))
        )
    ...
    def test_auth_on_delete_user_voicemail_message(self):
        assert_that(
            calling(self.calld_client.voicemails.delete_voicemail_message_from_user).with_args(
                'message_id'
            ),
            raises(CalldError).matching(has_properties(status_code=401))
        )
    ...
    def test_auth_on_get_user_voicemail_message(self):
        assert_that(
            calling(self.calld_client.voicemails.get_voicemail_message_from_user).with_args(
                'message_id'
            ),
            raises(CalldError).matching(has_properties(status_code=401))
        )
    ...
    def test_auth_on_put_user_voicemail_message(self):
        assert_that(
            calling(self.calld_client.voicemails.move_voicemail_message_from_user).with_args(
                'message_id', 'dest_folder_id'
            ),
            raises(CalldError).matching(has_properties(status_code=401))
        )
    ...
    def test_auth_on_get_user_voicemail_recording(self):
        assert_that(
            calling(self.calld_client.voicemails.get_voicemail_recording_from_user).with_args(
                'message_id'
            ),
            raises(CalldError).matching(has_properties(status_code=401))
        )
