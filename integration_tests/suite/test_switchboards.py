...
class TestSwitchboardCallsHeldAnswer(TestSwitchboards):
    ...
    def test_answer_same_held_call_twice_then_404(
        self,
    ):
        token = random_uuid(prefix='my-token-')
        user_uuid = random_uuid(prefix='my-user-uuid-')
        ...
        # the call has already been held by the first operator, so call not available
        assert_that(response.status_code, equal_to(404))

    def test_given_one_held_call_and_two_operators_when_two_operators_answer_then_404(
        self,
    ):
        # operator 1
        token = random_uuid(prefix='my-token-')
        ...
        response = self.calld.put_switchboard_held_call_answer_result(
            switchboard_uuid, answer_call_id, token2, line_id2
        )
        # the call has already been taken by the first operator, so call not available
        assert_that(response.status_code, equal_to(404))
