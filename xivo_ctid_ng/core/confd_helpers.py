...
class User(object):
    ...
    def main_line(self):
        try:
            user_lines_of_user = self._confd.users.relations(self._uuid).list_lines()['items']
        except HTTPError as e:
            if not_found(e):
                raise InvalidUserUUID(self._uuid)
            raise
        ...
        if not main_line_ids:
            raise UserHasNoLine(self._uuid)
...
class Line(object):
    ...
    def _get(self):
        try:
            return self._confd.lines.get(self._id)
        except HTTPError as e:
            raise
        except RequestException as e:
